...
for i in a.lower().split('\n'):  # Convert string into lower case and split it by '\n'
     for y in i.split('.'):  # Split again by '.'
          ...
          last_word = [i.replace(':', '') for i in y.split(' ') if i != '' and len(y) > 0 and not i.isdigit()]  # Create lists with every word (but not number) of every sentence, including empty (when '/n')
...
# print(output.count('\n') + output.count(' ')) another way of whitespaces count

last_words_sentance =  ' '.join(last_words) + '.'  # Create a new sentence(string) from list 'last_words'

output = output[:output.find('paragraph.')+10] + '\n' + last_words_sentance.capitalize() + output[output.find('paragraph.')+10:]  # Final output
